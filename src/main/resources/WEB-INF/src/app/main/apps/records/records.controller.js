(function() {
	...
	/** @ngInject */
	function RecordsController($scope, $mdSidenav, Owners, Records, Contacts, User, msUtils, $mdDialog, $document) {
		...
		vm.openRecordDialog = openRecordDialog;
		...
		function openContactDialog(ev, contact) {
			$mdDialog.show({
				...
				controllerAs: 'vm',
				templateUrl: 'app/main/apps/contacts/dialogs/contact/record-dialog.html',
		 ...
		 * @param contact
		 */
		function openRecordDialog(ev, record) {
			$mdDialog.show({
				controller: 'RecordDialogController',
				controllerAs: 'vm',
				templateUrl: 'app/main/apps/records/dialog/record/record-dialog.html',
				parent: angular.element($document.find('#content-container')),
				targetEvent: ev,
				clickOutsideToClose: true,
				locals: {
